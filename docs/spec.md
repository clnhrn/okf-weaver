...
- The exported artifact is a **conformant OKF v0.1 directory** (zipped): `index.md` with `okf_version` + bundle `name`, one `tables/<name>.md` concept file per table each carrying the required `type` in parseable YAML frontmatter and a `# Schema` table with FK cross-links.
...
- **Raw SQL DDL parser** — parses `CREATE TABLE` statements into `Table` / `Column` objects (name, type, nullability, primary keys, inline comments). Uses `sqlglot` for dialect-tolerant parsing across RDBMS dialects (Postgres, MySQL, SQL Server, warehouse SQL). **Foreign keys and primary keys** are extracted whether they are written inline (`col ... REFERENCES t(c)`), as table-level constraints (`FOREIGN KEY (...)` / `PRIMARY KEY (...)`), **or as separate `ALTER TABLE ... ADD CONSTRAINT` statements** — the last is how `pg_dump` and SQL Server/SSMS emit them, so a raw dump works without hand-editing. FKs become `Column.references = "table.column"` and travel through to the bundle, so relationships become part of the context automatically (no user effort).
...
- Renders the validated internal bundle to a **conformant OKF v0.1 directory** (per [GoogleCloudPlatform/knowledge-catalog `okf/SPEC.md`](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md)) and zips it:
  - `index.md` — bundle-root manifest carrying `okf_version: "0.1"` + a user-supplied `name` in frontmatter (the `name` also becomes the `#` title and the downloaded `.zip` filename, slugified; blank falls back to "OKF Bundle") and a linked listing of concepts (the one place `okf_version` is declared, per spec §5).
...
The single biggest accuracy lever is telling the model what the data _means_. `/api/generate` accepts optional free-text **`context`** (domain/business notes and a glossary — e.g. how "revenue" is defined, allowed `status` values). It is appended to the **system prompt** (so it is cache-friendly across the per-table calls in one request) under an "authoritative — prefer over guessing" heading, and directly lifts confidence on the ambiguous columns that otherwise score low. It is **optional by design** — the product's promise is good context from _minimal_ input, so context enriches rather than gates. The UI nudges toward filling it in — clicking Generate with an empty context field shows a one-click confirmation before proceeding — but this is presentation only: the `context` field remains fully optional in the API contract, and generation without it is a fully supported path. Raw sample data is deliberately **not** accepted in v1 to preserve the metadata-only privacy stance (see §9).
...
| Config                | env vars: `ANTHROPIC_API_KEY`, `OKF_MODEL_ID`, `OKF_ALLOWED_ORIGINS` (CORS) |
