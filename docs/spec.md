...
| `POST` | `/api/generate` | `{ schema: SchemaIR, context?: string }` — optional domain/glossary notes threaded into every per-table prompt | **SSE stream** — one `table` event per completed table (partial OKF + confidence), then a final `done` event with the assembled bundle + validation report |
