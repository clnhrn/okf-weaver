...
This repo is at **documentation / scaffold stage**. `docs/` holds the tracked design set — `spec.md` (architecture + AI module + acceptance criteria, the authoritative source), `PRD.md` (product scope), `idea-validation-report.md` (market validation). `backend/` and `frontend/` described below are being built out. When code and spec disagree, treat `docs/spec.md` as the source of truth and update it alongside the code. (The assignment brief lives untracked under `archive/`, which is gitignored.)
